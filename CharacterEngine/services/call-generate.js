// @ts-nocheck
// CharacterEngine 内置的 callGenerate 服务
...
const SOURCE_TAG = 'character-engine-cg';
...
class CallGenerateService {
    ...
    normalizeSessionId(rawId) {
        if (!rawId) return 'ce1';
        const m = String(rawId).match(/^ce(\d{1,2})$/i);
        if (m) {
    ...
    async _sendMessages(messages, options, requestId, sourceWindow) {
        const sessionId = this.normalizeSessionId(options?.session?.id || 'ce1');
...
// 导出服务类和工厂函数
export { CallGenerateService };

// 创建全局单例
let globalServiceInstance = null;

export function getCallGenerateService() {
    if (!globalServiceInstance) {
        globalServiceInstance = new CallGenerateService();
    }
...
export async function handleGenerateRequest(options, requestId, sourceWindow) {
    ...
    return await service.handleGenerateRequest(options, requestId, sourceWindow);
}